...
angular.module('twitReader.factories', [])
	   ...
	   {
	   		var PUBLIC_KEY = 'tB5T5hvlnp-QfKMKSsskGpTzNQE';
		   		...
		   		connectTwitter : function()
		   		{
	   				...
	   				OAuth.popup('twitter', {cache: true}, function(error, result)
	   				{
	   					...
	   					if(!error)
	   					{
	   						...
	   						$('div.status').addClass("text-success");
	   						$('#statusMessage').text("Successful!");
	   					}
	   					else
	   					{
	   						$('div.status').addClass("text-danger");
	   						$('#statusMessage').text("Authentification failed!");
	   					}
	   				});	
